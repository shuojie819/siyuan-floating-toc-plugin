...
34→
35→*   **⚡ 实时同步与配置**
36→    *   **秒级响应**：基于 WebSocket 深度集成，文档增删改（包括标题的移动、删除）大纲实时更新，零延迟。
37→    *   **聚焦模式支持**：完美支持思源的“聚焦 (Zoom In)”模式。当聚焦到某个标题块时，大纲自动切换为只显示当前聚焦范围内的内容。
38→    *   **智能导航**：在聚焦模式下点击大纲中不可见的标题（位于聚焦范围外），插件会自动退出聚焦模式并精准跳转到目标位置。
39→    *   **默认位置设置**：可在插件设置中自定义大纲默认显示在左侧还是右侧。
40→    *   **独立记忆**：文档内的位置调整（切换左右）仅对当前会话有效，不影响全局默认配置，给你最大的灵活度。
